...
// Explicit EXECUTE grants for the compendium RPCs (matches the spell/sphere search-RPC convention).
function printGrants() {
  console.log("-- 0024_pfcore_grants.sql  (generated by pfcore.mjs — explicit RPC execute grants)\n");
  for (const def of TABLES)
    if (def.browsable) console.log(`grant execute on function public.search_${def.table}(text, int) to anon, authenticated;`);
  console.log(`grant execute on function public.compendium_distinct(text, text) to anon, authenticated;`);
}
...
else { console.error("usage: node pfcore.mjs ddl|rpc|grants|load [table]|counts"); process.exit(1); }
