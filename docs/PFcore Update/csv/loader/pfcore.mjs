...
function ddlFor(def) {
  ...
  if (def.browsable) {
    out += `create or replace function public.search_${def.table}(p_query text default '', p_limit int default 60)\n`;
    out += `returns setof public.${def.table} language sql stable set search_path = public as $func$\n`;
    out += `  select * from public.${def.table}\n`;
    out += `  where p_query = '' or search @@ websearch_to_tsquery('english', p_query)\n`;
    out += `  order by case when p_query = '' then 0 else ts_rank(search, websearch_to_tsquery('english', p_query)) end desc, "${def.order}"\n`;
    out += `  limit greatest(1, least(p_limit, 200));\n$func$;\n`;
  }
  return out;
}
...
else { console.error("usage: node pfcore.mjs ddl|load [table]|counts"); process.exit(1); }
