...
def body_after(h):
    ...
    return re.split(r'id="catlinks"|<div class="printfooter"|NewPP limit', body)[0]
def features(body):
    out=[]
...
def progression(body):
    ...
    for r in rows[1:]:
        ...
        if not cs: continue
        if hdr and len(cs)==len(hdr): out.append(dict(zip(hdr,cs)))
        else: out.append(cs)
...
rows=[]
SETS=[("akashic","base","Akashic/Classes"),("akashic","prestige","Akashic/Prestige Classes"),
      ("psionic","base","Psionics/Classes"),("psionic","prestige","Psionics/Prestige Classes"),
      ("path_of_war","base","Path Of War/Disciplines")]  # PoW class-like pages filtered below
for sysname,ctype,folder in SETS:
    for p in sorted(glob.glob(os.path.join(CL,folder,"*.html"))):
        h=open(p,encoding='utf-8',errors='replace').read()
        if sysname=="path_of_war" and 'Associated Skill' in h: continue  # skip real disciplines
        ...
        rows.append({"name":name,"class_type":ctype,"system":sysname,
            "alignment":v.get("alignment",""),"hit_die":v.get("hitdie",""),
            "skill_points":v.get("skilleachlevel",""),"bab":v.get("bab",""),
            "fort":v.get("savefort",""),"ref":v.get("saveref",""),"will":v.get("savewill",""),
            "class_features":cell(", ".join(features(body))),
            "progression_json":progression(body).replace('\t',' '),
            "description":cell("<br><br>".join(p2 for p2 in (clean(mm.group(2)) for mm in re.finditer(r'<(p|dl)[^>]*>(.*?)</\1>',body,re.S)) if p2)[:1200] if False else "<br><br>".join([p2 for p2 in (clean(mm.group(2)) for mm in re.finditer(r'<(p|dl)[^>]*>(.*?)</\1>',body,re.S)) if p2][:6])),
            "source":cell(sb.group(1)) if sb else '',
            "url":canon(h,"https://metzo.miraheze.org/wiki/"+name.replace(' ','_'))})
# PoW class-like pages from Disciplines (Awakened Blade etc.)
with open(os.path.join(OUTDIR,"threepp_classes.tsv"),"w",encoding="utf-8",newline="") as f:
    f.write("\t".join(COLS)+"\n")
