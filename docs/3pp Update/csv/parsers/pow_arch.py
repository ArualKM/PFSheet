...
CL=os.path.join(ROOT,"3pp Classes","Path Of War"); OUTDIR=os.path.join(ROOT,"csv")
def scrub(s):
...
rows=[]
for p in sorted(glob.glob(os.path.join(CL,"Base PF Archetypes","*.html"))):
    ...
    paras=[x for x in (clean(mm.group(2)) for mm in re.finditer(r'<(p|dl)[^>]*>(.*?)</\1>',body,re.S)) if x]
    rows.append({"name":name,"base_class":v.get("class1",""),"system":"path_of_war",
        "altered_features":cell(", ".join(features(body))),"description":cell("<br><br>".join(paras)),
        "source":cell(sb.group(1)) if sb else '',"url":canon(h,"https://metzo.miraheze.org/wiki/"+name.replace(' ','_'))})
with open(os.path.join(OUTDIR,"_pow_arch_rows.tsv"),"w",encoding="utf-8",newline="") as f:
    for r in rows: f.write("\t".join(scrub(r.get(c,"")) for c in ACOLS)+"\n")
print(f"pow_archetypes={len(rows)}")
